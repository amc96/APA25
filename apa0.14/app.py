...
def download_cruzamentos_csv():
    ...
    try:
        ...
        resultado_graspe = grasp_cruzamentos(df, iteracoes=50, rcl_tamanho=3)
        melhores_cruzamentos = resultado_graspe['cruzamentos']
        ...
        # Adicionar estatísticas no final do CSV
        df_resultados.loc[len(df_resultados)] = ['ESTATÍSTICAS', '', '']
        df_resultados.loc[len(df_resultados)] = ['Total de cruzamentos', resultado_graspe['total_cruzamentos'], '']
        df_resultados.loc[len(df_resultados)] = ['Média dos coeficientes', f"{resultado_graspe['media_coeficientes']:.4f}", '']
        df_resultados.loc[len(df_resultados)] = ['Valor objetivo', f"{resultado_graspe['valor_objetivo']:.4f}", '']
