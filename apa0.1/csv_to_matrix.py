...
def csv_to_matrix(csv_path: str) -> pd.DataFrame:
    ...
    # Imprime informações básicas do DataFrame carregado
    print(f"DataFrame carregado com {df.shape[0]} linhas e {len(df.columns)} colunas")
    ...
    # Elimina duplicatas com base nas colunas Animal_1 e Animal_2, mantendo apenas a primeira ocorrência
    # Isso evita problemas de duplicação quando usamos pivot_table
    df = df.drop_duplicates(subset=['Animal_1', 'Animal_2'], keep='first')
    
    # Cria a matriz utilizando o pivot_table
    # Animal_1 nas colunas, Animal_2 nas linhas e os valores são os coeficientes
    matrix = df.pivot_table(index='Animal_2', columns='Animal_1', values='Coef')
